...
export const ZodNameString = z
  ...
  .trim()
  .regex(
    new RegExp(Constant.STRING_NUM_SPACE_PATTERN),
    "Only character, number and space are allowed"
  );
...
export const ZodNumericNonNegString = z
  ...
  .trim()
  .regex(
    new RegExp(Constant.STRING_NUM_PATTERN_NON_NEG),
    "Only positive number is allowed"
  );

export const ZodSimpleString = z.string().trim().nonempty();
export const ZodEmptyString = z.string().trim().optional();
